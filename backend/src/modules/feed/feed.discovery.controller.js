...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      const redisCachedResult = await redisCached(
        cacheKey,
        async () => {
...
router.get('/for-you', discoveryLimiter, optionalAuth, async (req, res) => {
  try {
    ...
    const forYouResult = await redisCached(
      forYouCacheKey,
      async () => {
        ...
        const scoredSheets = sheetCandidates
          ...
          .map((sheet) => {
            ...
            const isEnrolledCourse =
              sheet.course?.id != null && enrolledCourseSet.has(sheet.course.id)
            const isFollowedAuthor =
              sheet.author?.id != null && followedUserSet.has(sheet.author.id)
            let score = baseScore
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const discoverResult = await redisCached(
        `feed:course-discover:${courseId}:${limit}`,
        async () => {
