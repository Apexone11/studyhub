...
12. **Feature-flag seed scope MUST track the ship frontier, not the set of declared flags.** The client's `designV2Flags.js` hook fail-opens when a FeatureFlag row is missing (`FLAG_NOT_FOUND` → `true`) — which means a "seed every flag as enabled" script silently flips on every in-flight Phase-N surface the moment it runs. Rule: (a) SHIPPED features get an explicit `enabled=true` row in the seed; (b) IN-FLIGHT features get an explicit `enabled=false` row to opt out of fail-open until they ship; (c) when a phase ships, move its flag name from the in-flight list to the shipped list in the same commit that flips the gate. Don't batch-enable. Don't skip the in-flight row (the fail-open would expose the WIP). See `SHIPPED_DESIGN_V2_FLAGS` + `IN_FLIGHT_DESIGN_V2_FLAGS` in `backend/scripts/seedBetaUsers.js` for the canonical split.
