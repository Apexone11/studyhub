...
export default function SecurityTab() {
  ...
  if (loading) return <div style={{ padding: 24, color: 'var(--sh-muted)' }}>Loading security stats...</div>
  ...
  return (
    <section style={{ fontFamily: FONT, display: 'grid', gap: 18 }}>
      <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: 'var(--sh-heading)', display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
        ...
        <StatCard label="Failed Login Users" value={o.failedAttemptUsers} color="var(--sh-warning)" />
        ...
        <StatCard label="Pending Sheet Reviews" value={o.pendingSheetReviews} color="var(--sh-warning)" />
        <StatCard label="Pending Group Reports" value={o.pendingGroupReports} color="var(--sh-danger)" />
        <StatCard label="Waitlist (waiting)" value={o.pendingWaitlist} color="var(--sh-brand-accent)" />
        <StatCard label="Audit Actions (24h)" value={o.groupAuditActions24h} color="var(--sh-muted)" />
      </div>

      {/* Failed login accounts */}
      <div style={{ background: 'var(--sh-surface)', borderRadius: 14, border: '1px solid var(--sh-border)', padding: '16px 18px' }}>
        <h3 style={{ margin: '0 0 12px', fontSize: 14, fontWeight: 800, color: 'var(--sh-heading)', display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <thead>
                <tr>
                  {['Username', 'Failed Attempts', 'Locked', 'Last Attempt', 'Action'].map((h) => (
                    <th key={h} style={{ textAlign: 'left', padding: '6px 10px', borderBottom: '2px solid var(--sh-border)', color: 'var(--sh-muted)', fontSize: 11, fontWeight: 700, textTransform: 'uppercase' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {data.recentFailedAccounts.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td style={cellStyle}>
                      <span style={{ color: u.failedAttempts >= 5 ? 'var(--sh-danger)' : 'var(--sh-warning)', fontWeight: 700 }}>
                        {u.failedAttempts}
                      </span>
                    </td>
                    <td style={cellStyle}>
                    ...
                    <td style={cellStyle}>{u.lastAttempt ? new Date(u.lastAttempt).toLocaleString() : '—'}</td>
                    <td style={cellStyle}>
...
function StatCard({ label, value, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--sh-muted)', textTransform: 'uppercase', letterSpacing: '0.3px' }}>{label}</div>
...
const cellStyle = {
  padding: '6px 10px', borderBottom: '1px solid var(--sh-soft)', color: 'var(--sh-heading)',
}
