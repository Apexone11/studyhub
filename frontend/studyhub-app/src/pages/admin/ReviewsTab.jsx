...
function ReportCard({ report, onExpand, expanded }) {
  ...
  return (
    <div style={{
      ...
      >
        <div>
          <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--sh-heading)', marginBottom: 4 }}>
      ...
      {expanded && (
        <div style={{ padding: '0 20px 20px', borderTop: '1px solid var(--sh-border)' }}>
          {/* Stats bar */}
          <div style={{
            display: 'flex', gap: 12, marginTop: 16, marginBottom: 20, flexWrap: 'wrap',
          }}>
            <StatCard label="Reviews" value={report.reviewCount} color="var(--sh-info-text)" />
            <StatCard label="Avg Rating" value={`${report.averageStars.toFixed(1)}/5`} color="var(--sh-warning)" />
          ...
          {report.rawAnalysis && (
            <div style={{
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--sh-muted)', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                Executive Summary
              </div>
              <div style={{ fontSize: 13, color: 'var(--sh-text)', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>
                {report.rawAnalysis}
              </div>
            </div>
          )}
...
function StatCard({ label, value, color }) {
  return (
    <div style={{
      ...
      <div style={{ fontSize: 11, color: 'var(--sh-muted)', fontWeight: 600, marginTop: 2 }}>{label}</div>
    </div>
  )
}
...
function InsightList({ title, items, iconColor, bgColor, borderColor, icon }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 12, fontWeight: 700, color: iconColor, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '0.5px' }}>
        {title}
      </div>
      <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {items.map((item, i) => (
          <li key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', fontSize: 13, color: 'var(--sh-text)', lineHeight: 1.5 }}>
...
function AIReportsPanel() {
  ...
  const loadReports = useCallback(async () => {
    ...
    setError('')
    try {
      const res = await fetch(`${API}/api/reviews/admin/reports?limit=20`, { credentials: 'include' })
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--sh-muted)' }}>
            Generate an AI-powered analysis of user reviews to identify strengths, weaknesses, and actionable improvements.
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
      ...
      {loading ? (
        <div style={{ color: 'var(--sh-muted)', fontSize: 13, padding: 20 }}>Loading reports...</div>
      ) : !reports.length ? (
        <div style={{
          color: 'var(--sh-muted)', fontSize: 13, padding: 40, textAlign: 'center',
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button type="button" onClick={() => setView('reviews')} style={subTabStyle(view === 'reviews')}>
            Reviews
          </button>
          <button type="button" onClick={() => setView('reports')} style={subTabStyle(view === 'reports')}>
            AI Reports
          </button>
        </div>
      </div>
