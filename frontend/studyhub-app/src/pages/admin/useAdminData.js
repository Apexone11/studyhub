...
export function useAdminData() {
  ...
  const loadOverview = useCallback(async ({ signal } = {}) => {
    try {
      ...
      if (response.status === 403) {
        setOverview((current) => ({
          ...current, loading: false, loaded: current.loaded,
      ...
      setOverview((current) => ({
        loading: false, loaded: current.loaded,
        error: error.message || 'Could not load admin stats.', stats: current.stats,
      }))
    }
  }, [clearSession, navigate])
  ...
  const loadPagedData = useCallback(async (tab, page = 1) => {
    const stateSetters = {
      users: setUsersState, sheets: setSheetsState, 'sheet-reviews': setReviewState,
      announcements: setAnnouncementsState, 'deletion-reasons': setDeletionsState,
    ...
    try {
      ...
      const items = data.users || data.sheets || data.announcements || data.reasons || data.suppressions || []
      setState({ loading: false, loaded: true, error: '', page: data.page || page, total: data.total || items.length, items })
    } catch (error) {
      setState((current) => ({ ...current, loading: false, error: error.message || 'Could not load this tab.' }))
    }
  ...
  async function patchRole(userId, role) {
    await apiJson(`/api/admin/users/${userId}/role`, { method: 'PATCH', body: JSON.stringify({ role }) })
    await Promise.all([loadPagedData('users', usersState.page), loadOverview()])
  }

  function deleteUser(userId) {
    setConfirmAction({ title: 'Delete this user?', message: 'This will permanently remove the user and all their data. This cannot be undone.', variant: 'danger',
      onConfirm: async () => { setConfirmAction(null); await apiJson(`/api/admin/users/${userId}`, { method: 'DELETE' }); await Promise.all([loadPagedData('users', usersState.page), loadOverview()]) },
    })
  }
  ...
  async function saveAnnouncement(event) {
    ...
    setAnnounceError('')
    try {
      await apiJson('/api/admin/announcements', { method: 'POST', body: JSON.stringify(announceForm) })
      setAnnounceForm({ title: '', body: '', pinned: false })
      await loadPagedData('announcements', 1)
    } catch (error) { setAnnounceError(error.message || 'Could not save announcement.') }
  ...
  function deleteAnnouncement(announcementId) {
    setConfirmAction({ title: 'Delete this announcement?', message: 'This will permanently remove the announcement.', variant: 'danger',
      onConfirm: async () => { setConfirmAction(null); await apiJson(`/api/admin/announcements/${announcementId}`, { method: 'DELETE' }); await loadPagedData('announcements', announcementsState.page) },
    })
  }
  ...
  async function loadSuppressionAudit(suppressionId, page = 1) {
    ...
    try {
      ...
      setAuditState({ loading: false, loaded: true, error: '', page: data.page || page, total: data.total || 0, entries: data.entries || [], suppression: data.suppression || null, suppressionId })
    } catch (error) {
      setAuditState((current) => ({ ...current, loading: false, loaded: current.loaded, error: error.message || 'Could not load suppression audit.', suppressionId }))
    }
  }
  ...
  async function unsuppressRecipient(record) {
    ...
    if (reason.length < 8) { setUnsuppressErrorById((c) => ({ ...c, [record.id]: 'Provide an unsuppress reason with at least 8 characters.' })); return }
    setSuppressionMessage('')
    setUnsuppressSavingId(record.id)
    setUnsuppressErrorById((c) => ({ ...c, [record.id]: '' }))
    try {
      await apiJson(`/api/admin/email-suppressions/${record.id}/unsuppress`, { method: 'PATCH', body: JSON.stringify({ reason }) })
      setUnsuppressReasonById((c) => ({ ...c, [record.id]: '' }))
      const followUp = [loadPagedData('email-suppressions', suppressionsState.page), loadOverview()]
      if (auditState.suppressionId === record.id) followUp.push(loadSuppressionAudit(record.id, auditState.page))
      await Promise.all(followUp)
      setSuppressionMessage('Recipient unsuppressed successfully.')
    } catch (error) { setUnsuppressErrorById((c) => ({ ...c, [record.id]: error.message || 'Could not unsuppress recipient.' })) }
    finally { setUnsuppressSavingId(null) }
  }

  return {
    user, overview, usersState, sheetsState, reviewState, announcementsState, deletionsState,
    suppressionsState, suppressionStatus, suppressionQueryInput, suppressionQuery,
    suppressionMessage, unsuppressReasonById, unsuppressErrorById, unsuppressSavingId,
    auditState, reviewStatus, reviewFormatFilter, reviewScanFilter, announceForm, announceSaving,
    announceError, confirmAction, reviewPanelSheetId, htmlKillSwitch, htmlToggleSaving,
    setSuppressionStatus, setSuppressionQueryInput, setSuppressionMessage, setSuppressionsState,
    setUnsuppressReasonById, setUnsuppressErrorById, setReviewStatus, setReviewFormatFilter,
    setReviewScanFilter, setReviewState, setAnnounceForm, setConfirmAction, setReviewPanelSheetId,
    setAuditState,
    apiJson, loadOverview, loadPagedData, loadHtmlKillSwitch, toggleHtmlUploads,
    patchRole, deleteUser, deleteSheet, reviewSheet, saveAnnouncement, togglePin,
    deleteAnnouncement, loadSuppressionAudit, submitSuppressionSearch, clearSuppressionFilters,
    unsuppressRecipient,
  }
}
