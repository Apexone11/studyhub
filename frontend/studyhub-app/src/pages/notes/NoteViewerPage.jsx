...
export default function NoteViewerPage() {
  ...
  const userReaction = reactionState?.userReaction !== undefined ? reactionState.userReaction : (note?.userReaction ?? null)
  ...
  if (loading) {
    return (
      <div style={{ fontFamily: PAGE_FONT, padding: 40, textAlign: 'center', color: 'var(--sh-muted)' }}>
        Loading...
      </div>
    )
  }
    ...
    <div style={{ fontFamily: PAGE_FONT, maxWidth: 820, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
          <Link to="/notes" style={{ color: 'var(--sh-muted)', textDecoration: 'none', fontSize: 13 }}>
            Notes
          </Link>
          <span style={{ color: 'var(--sh-slate-300)' }}>/</span>
        </div>

        <h1 style={{ fontSize: 26, fontWeight: 800, color: 'var(--sh-heading)', margin: '0 0 12px' }}>
          {note.title}
        </h1>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', fontSize: 13, color: 'var(--sh-subtext)' }}>
          {note.author && (
            <Link
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 8, color: 'var(--sh-info-text)', textDecoration: 'none', fontWeight: 600 }}
            >
      ...
      {!note.private && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          {/* Like */}
          {user && (
            <button type="button" onClick={() => handleReact('like')}
          ...
          {/* Dislike */}
          {user && (
            <button type="button" onClick={() => handleReact('dislike')}
      ...
      {note.private && (
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {note.isOwner && (
            <Link to={`/notes?select=${note.id}`} style={{ ...actionBtnStyle(false, 'var(--sh-info)'), textDecoration: 'none' }}>
              Open in Editor
            </Link>
          )}
        </div>
