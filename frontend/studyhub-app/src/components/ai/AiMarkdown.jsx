...
function renderInline(text) {
  ...
  while (remaining.length > 0) {
    ...
    if (codeMatch) {
      parts.push(
        <code key={key++} style={{
          background: 'var(--sh-soft)', padding: '1px 5px', borderRadius: 4,
          fontSize: '0.9em', fontFamily: 'monospace', color: 'var(--sh-brand)',
        }}>{codeMatch[1]}</code>
      )
...
export default function AiMarkdown({ content }) {
  ...
  while (i < lines.length) {
    ...
    if (line.match(/^```/)) {
      ...
      nodes.push(
        <div key={i} style={{
          background: 'var(--sh-slate-900)', borderRadius: 10, padding: '12px 14px',
