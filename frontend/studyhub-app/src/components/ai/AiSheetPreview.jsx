...
export function SheetPreviewBar({ html, conversationTitle }) {
  ...
  const handlePublish = async () => {
    try {
      const title = conversationTitle || 'AI-Generated Study Sheet'
      const res = await fetch(`${API}/api/sheets`, {
        ...
        body: JSON.stringify({
          title,
          description: `Generated by Hub AI`,
          htmlContent: html,
          private: true, // Start as private draft
        }),
      })
      if (!res.ok) throw new Error('Failed to create sheet')
      const sheet = await res.json()
      navigate(`/sheets/${sheet.id}/lab`)
    } catch {
...
<style>
  body { font-family: 'Plus Jakarta Sans', system-ui, sans-serif; padding: 24px; margin: 0; color: #1a1a2e; line-height: 1.7; }
