...
## Cycle S-9 (Tasks 9-11) — Trust Levels: On-Login Promotion, Admin Override, Case Filtering [2026-03-26]

### Added

- **Task 9 — On-login auto-promotion** (`backend/src/modules/auth/auth.login.controller.js`):
  - Added `require('../../lib/trustGate')` import.
  - After `issueAuthenticatedSession`, added fire-and-forget `void checkAndPromoteTrust(user.id)` call.
  - Non-blocking: response is already sent; promotion happens asynchronously. The user sees the updated trust level on their next authenticated request.

- **Task 9 — trustLevel in /me response** (`backend/src/modules/auth/auth.service.js`):
  - Added `trustLevel: true` to `getAuthenticatedUser()` Prisma select.
  - Added `trustLevel: user.trustLevel || 'new'` to `buildAuthenticatedUserPayload()` output.
  - `GET /api/auth/me` and login response now both include `trustLevel` in the user object.

- **Task 10 — Admin PATCH /api/admin/users/:id/trust-level** (`backend/src/modules/admin/admin.users.controller.js`):
  - Added `logModerationEvent` import from `../../lib/moderationLogger`.
  - New endpoint validates `trustLevel` is one of `new`, `trusted`, `restricted`.
  - Guards super admin from being restricted (`SUPER_ADMIN_PROTECTED` code).
  - Sets `trustedAt` to `new Date()` when promoting to `trusted`; nulls it when reverting to `new`.
  - Logs `trust_level_changed` moderation event with `newTrustLevel` metadata.
  - Returns updated user: `{ id, username, trustLevel, trustedAt }`.

- **Task 11a — trustLevel filter on GET /moderation/cases** (`backend/src/modules/moderation/moderation.admin.cases.controller.js`):
  - Added `if (req.query.trustLevel)` clause to build `where.user = { trustLevel }` filter.
  - Allows admin queue to show only `new` user cases, e.g. `?status=pending&trustLevel=new`.

- **Task 11b — trustLevel in case user relation**:
  - Updated `GET /cases` `include.user` select to `{ id, username, avatarUrl, trustLevel }`.
  - Updated `GET /cases/:id` `include.user` select to `{ id, username, avatarUrl, trustLevel }`.
  - Case list and detail responses now expose the subject user's trust level.

- **Test harness fix** (`backend/test/auth.routes.test.js`):
  - Added `trustGate` mock (`checkAndPromoteTrust` returns `{ promoted: false, trustLevel: 'new' }`).
  - Added `require.resolve('../src/lib/trustGate')` to `mockTargets` map.
  - Eliminates 12 unhandled-rejection noise events from fire-and-forget calls hitting the unmocked prisma in test environment.

### Validation

- `npx vitest run test/auth.routes.test.js test/admin.routes.test.js test/moderationReporting.test.js test/moderationVisibility.test.js`
  - 4 test files passed, 30 tests passed, 0 errors.

### Known Risks / Deferred

- `checkAndPromoteTrust` is fire-and-forget — if it throws after the response is sent, the error is silently swallowed (Prisma errors will not surface to the caller). This matches the pattern used by `scanContent` throughout the codebase.
- Admin trust-level override does not trigger a re-evaluation of existing queued moderation cases for the user. Cases already in the queue retain their pending status regardless of the new trust level.
